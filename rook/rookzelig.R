...
zelig.app <- function(env){
	...
	if(!warning){
        if(production){
            ...
            print('-----  rookzelig.R - file path adjustment:)');

            file_input = paste(NON_PRODUCTION_DATA_PATH, "fearonLaitin.tsv", sep="/")
            mydata <- read.delim(file_input)

            logline = paste("mydata <- read.delim(", file_input, ")", sep="")
            write(logline, mylogfile, append=TRUE)
            #mydata <- read.delim("../static/data/fearonLaitin.tsv")
            #write("mydata <- read.delim(\"../static/data/fearonLaitin.tsv\")",mylogfile,append=TRUE)
